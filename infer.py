...
from src.algorithm.noise import add_noise
import time
from algorithm.tone import (
    to_grayscale,
    binarize,
    otsu_binarize,
    apply_colormap,
)
from src.algorithm.tone import adjust_brightness, adjust_contrast, histogram
from algorithm.filter import (
    mean_filter,
    median_filter,
    gaussian_blurring,
    bilateral_filter_manual,
)
from src.algorithm.artistic import emboss, frosted
...
def main():
    ...
    mean_image = mean_filter(gaussian_image.copy(), kernel_size=3)
    median_image = median_filter(gaussian_image.copy(), kernel_size=3)
    gray_image = to_grayscale(image.copy())
    binary_image = binarize(gray_image.copy(), threshold=128)
    otsu_binary_image = otsu_binarize(gray_image.copy())
    ...
    save_image(otsu_binary_image, "data/otsu_binary_image.png")
